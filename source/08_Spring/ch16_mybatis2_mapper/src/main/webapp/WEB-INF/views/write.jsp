	...
	<form action="write.do" method="post" name="frm">
		...
		<tr>
		     ...
		     <select name="mgr">
		       <c:forEach items="${managerList }" var="mgrEmp">
		         <option value="${mgrEmp.empno }"
		           <c:if test="${mgrEmp.empno eq emp.mgr }">sekected="selected"</c:if>
		 ...
		 <tr>
		   <th>급여</th>
		   <td>
		     <input type="number" name="sal" value="${emp.sal }" required="required"></td>
		  </tr>
		  <tr>
		   <th>상여</th>
		   <td>
		     <input type="number" name="comm" value="${emp.comm }" required="required"></td>
		  </tr>
		  <tr>
		    <th>부서번호</th>
		    <td>
