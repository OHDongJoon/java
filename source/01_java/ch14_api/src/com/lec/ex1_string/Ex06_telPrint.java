...
// 전화번호 받아, 입력받은 전화번호, 짝수번호문자 , 꺼꾸로, 전화번호 앞자리, 전화번호뒷자리
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			for (int i = 0; i < tel.length(); i++) {
				if (i % 2 == 0) {
					System.out.print(tel.charAt(i));// 짝수번째문자열만출력
				} else {
					System.out.print(' ');// 홀수 번째 문자열 대신 ' ' 출력
				}
			}
			System.out.println();// 개행
			System.out.println("문자열 꺼꾸로");
			...
			System.out.print("전번 앞자리 : " + pre);
			System.out.print("전번 뒷자리 :" + post);
			System.out.println("전번 중간:" + mid);
		}
	}
}