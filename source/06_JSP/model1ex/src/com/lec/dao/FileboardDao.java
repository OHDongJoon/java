...
public class FileboardDao {
	...
	public ArrayList<FileboardDto> listBoard(int startRow , int endRow){
		ArrayList<FileboardDto> dtos = new ArrayList<FileboardDto>(); //리턴 타입 어레이 만들고 
		Connection conn = null;
	...
	public int getFileboardCnt() {
		...
		try {
			...
			rs = pstmt.executeQuery();
			rs.next();
			totCnt = rs.getInt(1); // 엘리아스 없으니  그냥 1번줄가져와 
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		} finally {
			try {
	...
	// 4. 답변글 쓰기전 step A -- ?번글에 대한 답변글 쓰기 전 작업
	private void preReplyStepA(int fref , int fre_step) { // 결과값 리턴 없이 바로 호출하게 void 
		Connection 		  conn = null;
		PreparedStatement pstmt = null;
	...
	// 5. 답변글 쓰기
			public int reply(FileboardDto dto) {
				...
				// cid, fsubject, fcontent, ffilename, fpw, fip : 내가 지금 쓸 답변글의 정보 
				preReplyStepA(dto.getFref(), dto.getFre_step()); // 답변글 쓰기전  a 스텝호출
				int result = FAIL;
				Connection 		  conn = null;
				PreparedStatement pstmt = null;
				String sql = "INSERT INTO FILEBOARD (fNUM , CID, fSUBJECT, fCONTENT, fFILENAME, fPW, fREF, fRE_STEP, fRE_LEVEL, fIP)" + 
			...
			public FileboardDto getBoard(int fnum) {
				...
				try {
					conn = getConnection();
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, fnum); // sql문 쏜다!
					rs = pstmt.executeQuery();
					if(rs.next()) { // 한줄만 오니 if 문 여러줄은 while
						  // int      fnum;
						 	String   cid  		= rs.getString("cid");
						    String  fsubject 	= rs.getString("fsubject");
						    String  fcontent 	= rs.getString("fcontent");
		...
		public int updateBoard(FileboardDto dto) {
			...
			try {
				...
				result = pstmt.executeUpdate();
				System.out.println(result==SUCCESS ? "글수정 성공" : "해당글번호가 없습니둥");
			} catch (SQLException e) {
	...
	//9. 글 삭제 DELETE FROM FILEBOARD WHERE fNUM=1 AND fPW='111'
		public int deleteBoard(int fnum, String fpw) {
			...
			try {
				...
				result = pstmt.executeUpdate();
				System.out.println(result==SUCCESS ? "글삭제 성공" : "글 삭제 실패");
			} catch (SQLException e) {
