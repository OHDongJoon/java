...
public class BoardDao {
	...
	//싱글톤
	private static BoardDao instance = new BoardDao(); // 내자신을 가르키는 변수
	...
	//2. 글목록 : // <bordDto>를 리턴
	 ...
	 public ArrayList<BoardDto> listBoard(int startRow, int endRow){ // listBoard 매개변수 있는거 
		 ArrayList<BoardDto> dtos = new ArrayList<BoardDto>();
		 Connection conn = null;
		 PreparedStatement pstmt = null;
		 ResultSet rs = null;
	...
	//4. 글번호로 글 한줄 가져오기 :
	 public BoardDto getBoardOneLine(int num) { //글번호가 있어 ? if 없어 ?else
		 BoardDto dto = null;
		 Connection conn = null;
		 PreparedStatement pstmt = null;
		 ResultSet rs = null;
	 ...
	 public BoardDto getBoardOneLine(String num) { //글번호가 있어 ? if 없어 ?else
		 BoardDto dto = null;
		 Connection conn = null;
		 PreparedStatement pstmt = null;
		 ResultSet rs = null;
	...
	//7. 글삭제 : 
	 ...
	 //8. 답변글 저장전 step ⓐ   0행 업데이트 뜰수 있으니 void :UPDATE BOARD SET RE_STEP=RE_STEP+1 WHERE REF=? AND RE_STEP > 1;
	 ...
	 public int reply(BoardDto dto) {
		...
		try {
			...
			System.out.println(result == SUCCESS ? "답변글성공 - " + dto  : "답변글실패 -" + dto);
		}catch (SQLException e) {
