...
public class BookDao {
	...
	public int insertBook(BookDto book) {
		...
		try {
			...
			pstmt.setString(4, book.getBimage1());
	...
	public ArrayList<BookDto> listBook() {
		...
		try {
			...
			while (rs.next()) {
				...
				String bimage2 = rs.getString("bimage1");
				;
...
// 책목록(paging : startRow ~ endRow)
	...
	public int getBookTotCnt() {
		...
		try {
			...
			rs = pstmt.executeQuery();
			rs.next();
			bookTotalCnt = rs.getInt("cnt"); // sql문에있는 count(*)cnt  엘리아스 놉 이면 rs.getInt(1)
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		} finally {
			try {
	...
	public BookDto getBook(int bid) {
		...
		try {
			...
			pstmt.setInt(1, bid);
			rs = pstmt.executeQuery();
			if(rs.next()) { // 있으면 없으면 최악조건 설정한 null return 
				String 	btitle 	  = rs.getString("btitle");
				int 	bprice 	  = rs.getInt("bprice");
				String 	bimage1   = rs.getString("bimage1");
				String 	bimage2   = rs.getString("bimage1");
